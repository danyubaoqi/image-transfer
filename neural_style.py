...
STYLE_SCALE = 1.0
ITERATIONS = 1000
VGG_PATH = 'D:\download\imagenet-vgg-verydeep-19.mat'
POOLING = 'max'
#需要在命令行运行 并定义参数，如风格图是哪个，内容图是哪个等
def build_parser():
    parser = ArgumentParser()
    parser.add_argument('--content',
            dest='content', help='content image',
            metavar='CONTENT', required=True,default="image1/content.jpg")
    parser.add_argument('--styles',
            dest='styles',
            nargs='+', help='one or more style images',
            metavar='STYLE', required=True,default="image1/style.jpg")
    parser.add_argument('--output',
            dest='output', help='output path',
            metavar='OUTPUT', required=True,default="out.jpg")
...
def main():
    ...
    if not os.path.isfile(options.network):#判断模型是否存在
        parser.error("Network %s does not exist. (Did you forget to download it?)" % options.network)

    content_image = imread(options.content)#读取内容图
...
#读取可能多个的内容图
    ...
    style_blend_weights = options.style_blend_weights#过去风格的权重
